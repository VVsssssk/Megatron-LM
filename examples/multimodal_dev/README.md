...
`--use-packed-sequence` for Megatron THD packing; optional
`--packing-buffer-size` only lets Energon pre-combine short samples before
the forward step builds final `PackedSeqParams`.
