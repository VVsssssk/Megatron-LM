...
class TransformerConfig(ModelParallelConfig):
    ...
    moe_flex_dispatcher_backend: Literal[
        'deepep', 'deepepv2', 'hybridep', 'ncclep', 'moonep'
    ] = "deepep"
    """[Experimental] The backend to use for flex token dispatcher. The default is "deepep".
    Options are "deepep", "deepepv2", "hybridep", "ncclep", and "moonep". Currently only
    "hybridep" backend supports the MNNVL case. "ncclep" uses NVIDIA NCCL Expert Parallelism via
    TransformerEngine's transformer_engine.pytorch.ep API. "moonep" is a single-node NVLink
    backend using an externally installed MoonEP package."""
    ...
    backends (deepep, deepepv2, hybridep, ncclep, moonep). None lets each backend use its own
    default. Unifies the deprecated per-backend moe_{deepep,hybridep}_num_sms knobs (routed in
    __post_init__)."""
    ...
    def __post_init__(self):
        ...
        if self.moe_enable_deepep:
            ...
            if self.moe_flex_dispatcher_backend in ("deepepv2", "hybridep", "moonep"):
