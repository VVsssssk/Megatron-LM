...
def _parse_te_expert_idx(key: str, module_name: str) -> Optional[int]:
    ...
    for kind in ("weight", "bias"):
        ...
        if index != suffix and index.isdigit():
            return int(index)
    # TE emits one extra-state object per grouped GEMM. The first has no suffix.
    if suffix.startswith("_extra_state"):
        index = suffix.removeprefix("_extra_state")
        if index == "":
            return 0
        if index.isdigit():
            return int(index)
...
class TEGroupedMLP(MegatronModule):
    ...
    def sharded_state_dict(
        ...
        ultraep_active = num_local_checkpoint_experts < self.num_local_experts
        for name, module in self._modules.items():
            ...
            if ultraep_active:
                sub_sd = self._ultraep_filter_replica_checkpoint_entries(
                    sub_sd,
                    name,
                    len(sharded_offsets),
    ...
    def _ultraep_filter_replica_checkpoint_entries(
        self,
        sub_sd: dict,
        module_name: str,
        ep_axis: int,
        ...
        *,
        ...
        """Drop ephemeral replicas and correct logical-master checkpoint metadata."""
