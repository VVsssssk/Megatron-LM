...
def _parse_te_expert_idx(key: str, module_name: str) -> Optional[int]:
    ...
    for kind in ("weight", "bias"):
        ...
        if index != suffix and index.isdigit():
            return int(index)
    # GroupedLinear owns one module-level extra state. It is replicated across
    # EP ranks and must not be filtered or reshaped as an expert shard.
    return None
...
class TEGroupedMLP(MegatronModule):
    ...
    def _ultraep_filter_replica_checkpoint_entries(
        ...
        *,
        ...
        """Drop ephemeral replicas and correct master expert checkpoint metadata."""
