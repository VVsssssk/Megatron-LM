...
class WeightDirection(Enum):
    """Weight orientation used by the push and pointer table."""
...
class _VirtualExperts:
    """One MoE layer's runtime weights and pointer tables over shared virtual slots.
    ...
    def __init__(self, group, config: _VirtualExpertConfig, parameters) -> None:
        if any(getattr(p, "is_gtp_weight_remat", False) for weights in parameters for p in weights):
            raise ValueError(
                "This virtual-expert port does not support GTP-sharded expert weights."
            )
        cls = type(self)
    ...
    def _allocate_shared(cls, group, config: _VirtualExpertConfig, templates) -> None:
        ...
        # virtual-expert partials, and autograd hands it on.
        cls.native_staging = tuple(
            torch.empty((count, *shape), dtype=config.grad_dtype, device=device)
    ...
    def weight_tables(self, direction: WeightDirection) -> tuple[torch.Tensor, ...]:
        """Wait for DDP's parameter gather before reading the optimizer-owned weights."""
        if direction == WeightDirection.FORWARD:
            for parameters in self.parameters:
                ensure_params_ready(parameters)
        return tuple(
            self.get_weight_table(i, direction, parameters)
            for i, parameters in enumerate(self.parameters)
        )

    def hand_off_wgrads(self, fc_layer: int) -> tuple[torch.Tensor, ...]:
        """Accumulate completed native and virtual gradients into optimizer-owned parameters."""
...
class VirtualExpertLoadBalancer:
    ...
    ``virtual_expert_triton.py``, scheduled from here. This port requires unsharded expert weights.
    """
    ...
    def prepare_expert_forward(self) -> None:
        """Wait for the forward weight push immediately before the expert GEMMs."""
    ...
    def _prepare_expert_backward(self) -> None:
        """Wait for the backward weight push before the expert backward GEMMs."""
        self._wait_weight_push()
    ...
    def _finish_grad_reduce(self) -> tuple[torch.Tensor, ...]:
        """Wait on the compute stream and return gradients in source-parameter order."""
        plan, self._plan = self._plan, None
