...
class _UltraEPReplicaGradReduce(torch.autograd.Function):
    ...
    def forward(ctx, hidden_states, moe_layer, virtual_layer_id):
        """Save the UltraEP context while forwarding expert inputs unchanged."""
...
class _UltraEPRestoreReplicaWeights(torch.autograd.Function):
    ...
    def forward(ctx, output, moe_layer, virtual_layer_id):
        """Save the UltraEP context while forwarding expert outputs unchanged."""
        ctx.moe_layer = moe_layer
        ctx.virtual_layer_id = virtual_layer_id
        return output
    ...
    @staticmethod
    def backward(ctx, grad_output):
        """Restore replica weights before evaluating expert input gradients."""
        not_none(ctx.moe_layer.ultraep_manager).weight_sync(
            ctx.virtual_layer_id, async_finish=False
        )
        return grad_output, None, None
...
class MoELayer(BaseMoELayer):
    ...
    def __init__(
        ...
        self.ultraep_layer_id: int | None = None
        if self.ultraep_enabled:
            ...
            self.ultraep_layer_id = self.ultraep_manager.layer_id(
    ...
    def _ultraep_register_replica_parameters(self) -> None:
        ...
        # Checkpoints contain logical experts only; replicas are transient cache state.
    ...
    def _ultraep_register_master_experts(self) -> None:
        ...
        for module_index, linear in enumerate((self.experts.linear_fc1, self.experts.linear_fc2)):
            for expert_index in range(manager.num_local_master_experts):
                ...
                if not hasattr(weight, "main_grad"):
                    raise RuntimeError(
                        "UltraEP master expert registration must run after MCore DDP initialization."
                    )
                if weight.main_grad.dtype != torch.float32:
                    raise TypeError("UltraEP requires FP32 main gradients; enable grad_reduce_in_fp32.")
        ...
        manager.register_master_experts(
            not_none(self.ultraep_layer_id),
            master_weights[0],
            master_weights[1],
            master_grads[0],
            master_grads[1],
