...
def _close_fds(fds) -> None:
    ...
    for fd in {int(fd) for fd in fds}:
        os.close(fd)


def _map_moonep_chunks(*, chunk_shape, dtype, fds, local_rank, world_size):
    """Map MoonEP VMM chunks across old ``fds`` and new ``shareables`` APIs."""
    fd_values = [int(fd) for fd in fds]

    def map_with_shareables():
        shareables = torch.tensor(fd_values, dtype=torch.int64)
        return _moonep_nvl_dist_map(
    ...
    doc = getattr(_moonep_nvl_dist_map, "__doc__", "") or ""
    if "shareables" in doc:
        try:
            return map_with_shareables()
        except TypeError as exc:
            if "fds" not in str(exc) and "incompatible function arguments" not in str(exc):
                raise

    try:
        return map_with_fds()
    except TypeError as exc:
        if "shareables" not in str(exc):
            raise
        return map_with_shareables()


def _moonep_supports_keyword(callable_obj, keyword: str) -> bool:
    """Return whether a MoonEP callable accepts ``keyword``."""
    func = getattr(callable_obj, "__func__", callable_obj)
    key = (func, keyword)
    if key in _moonep_keyword_support_cache:
        return _moonep_keyword_support_cache[key]
    ...
    _moonep_keyword_support_cache[key] = supported
    return supported


def moonep_supports_external_hidden_buffer(buffer) -> bool:
    """Return whether this MoonEP Buffer supports caller-owned hidden buffers."""
    return (
        _moonep_supports_keyword(buffer.dispatch, "hidden_buffer")
        and _moonep_supports_keyword(buffer.combine, "hidden_buffer")
    )


def _call_moonep_buffer_dispatch(
    buffer,
    hidden_states,
    route_weights=None,
    topk_experts=None,
    ...
    *,
    ...
    zero_copy_weights: bool = False,
    hidden_buffer=None,
):
    """Call MoonEP Buffer.dispatch across older and current keyword APIs."""
    dispatch = buffer.dispatch
    supports_hidden_buffer = _moonep_supports_keyword(dispatch, "hidden_buffer")
    use_zero_copy = bool(zero_copy)
    if hidden_buffer is not None and not supports_hidden_buffer:
        use_zero_copy = False

    args = [hidden_states]
    if (
        route_weights is not None
        or topk_experts is not None
        or tokens_per_expert is not None
    ):
        args.extend([route_weights, topk_experts, tokens_per_expert])

    kwargs = {}
    if plan is not None:
        kwargs["plan"] = plan
    if _moonep_supports_keyword(dispatch, "zero_copy"):
        kwargs["zero_copy"] = use_zero_copy
    if supports_hidden_buffer and hidden_buffer is not None:
        kwargs["hidden_buffer"] = hidden_buffer
    if _moonep_supports_keyword(dispatch, "zero_copy_weights"):
        kwargs["zero_copy_weights"] = zero_copy_weights
    if _moonep_supports_keyword(dispatch, "router_weights_zero_copy"):
        kwargs["router_weights_zero_copy"] = zero_copy_weights
    return dispatch(*args, **kwargs)


def _call_moonep_buffer_combine(
    buffer,
    *,
    plan,
    hidden_nvsh,
    ...
    """Call MoonEP Buffer.combine across older and current keyword APIs."""
    combine = buffer.combine
    supports_hidden_buffer = _moonep_supports_keyword(combine, "hidden_buffer")
    use_zero_copy = bool(zero_copy)
    if hidden_buffer is not None and not supports_hidden_buffer:
        use_zero_copy = False

    kwargs = {"plan": plan, "hidden_nvsh": hidden_nvsh}
    if route_weights_nvs is not None or _moonep_supports_keyword(combine, "route_weights_nvs"):
        kwargs["route_weights_nvs"] = route_weights_nvs
    if _moonep_supports_keyword(combine, "zero_copy"):
        kwargs["zero_copy"] = use_zero_copy
    if supports_hidden_buffer and hidden_buffer is not None:
        kwargs["hidden_buffer"] = hidden_buffer
    if _moonep_supports_keyword(combine, "router_weights_zero_copy"):
        kwargs["router_weights_zero_copy"] = False
...
def _allocate_moonep_mapping(*, chunk_shape, dtype, group, with_reduce_view: bool):
    ...
    if slot_fds is not None:
        ...
        reduce_view = _map_moonep_chunks(
            chunk_shape=list(chunk_shape),
            dtype=dtype,
            fds=slot_fd_list,
            local_rank=rank,
...
class MoonEPDispatch(torch.autograd.Function):
    ...
    def forward(
        ...
        try:
            dispatched, dispatched_probs, cu_seqlens, plan = _call_moonep_buffer_dispatch(
                buffer,
    ...
    def backward(ctx, grad_hidden, grad_probs, _grad_tokens_per_expert):
        ...
        try:
            ...
            grad_hidden_states, grad_topk_probs, _ = _call_moonep_buffer_combine(
                ctx.buffer,
                plan=ctx.plan,
...
class MoonEPCombine(torch.autograd.Function):
    ...
    def forward(ctx, expert_output, buffer, plan, bridge, fwd_hidden_buffer):
        ...
        combined, _, _ = _call_moonep_buffer_combine(
            buffer,
            plan=plan,
    ...
    def backward(ctx, grad_output):
        ...
        grad_expert_output, _, _, _ = _call_moonep_buffer_dispatch(
            ctx.buffer,
